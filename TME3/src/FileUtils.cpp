...
void pr::printResults(
    size_t total_words, size_t unique_words, std::vector<std::pair<std::string, int>> freq_pairs,
...
void pr::processRange(
    const std::string &file, std::streamoff start, std::streamoff end,
    std::function<void(const std::string &)> onWordEncountered
) {
  ...
  while (current_pos < end) {
    ...
    // std::cerr << "current_pos=" << current_pos << ", remaining=" << remaining << ", to_read=" <<
    // to_read << std::endl;
    ...
    if (bytes_read == to_read && current_pos + bytes_read < end &&
      ...
      if (last_space != std::string::npos) {
        ...
        // std::cerr << "Debug: current_pos=" << current_pos << ", buffer.size()=" << buffer.size()
        // << ", buffer.back()=" << (int)(unsigned char)buffer.back() << " '" << buffer.back() <<
        // "'" << std::endl; std::cerr << "No space in buffer. First 100: " << buffer.substr(0,100)
        // << std::endl; std::cerr << "Last 100: " << buffer.substr(buffer.size()-100) << std::endl;
