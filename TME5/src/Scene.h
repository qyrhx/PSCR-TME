...
class Scene {
 ...
 public:
  ...
  Scene(int width = 800, int height = 600)
    ...
    for (int y = 0; y < height; y++) {
      ...
      for (int x = 0; x < width; x++) {
        Vec3D offset =
            (x - width / 2.0) * pixelSizeX * right + (y - height / 2.0) * pixelSizeY * up;
  ...
  void addLight(const Vec3D &l) { lights.push_back(l); }
  ...
  Color computeColor(const Sphere &obj, const Ray &ray) const {
    ...
    for (const auto &light : lights) {
      ...
      if (obj.intersects(Ray(light, intersection)) >=
          tolight.length() - 1e-4) {  //  epsilon 1e-4 for double issues
        dt += tolight.normalize() &
              normal;  // l'angle (scalaire) donne la puissance de la lumiere reflechie
      }
    }
    // eclairage total
