...
class Color {
  unsigned char
      b;  // Deliberate order: b, g, r for BGR memory layout to allow direct fwrite in BMP export
  unsigned char g;
  unsigned char r;

 public:
  constexpr Color(unsigned char red = 255, unsigned char green = 255, unsigned char blue = 255)
  ...
  Color operator*(double ratio) {
    ...
    return Color(
        (unsigned char) (double(r) * ratio), (unsigned char) (double(g) * ratio),
        (unsigned char) (double(b) * ratio)
    );
  }
  ...
  Color operator+(const Color &c2) const {
    return Color(
        std::min(255, (int) r + c2.r), std::min(255, (int) g + c2.g), std::min(255, (int) b + c2.b)
    );
  }
};
