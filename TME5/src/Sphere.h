...
class Sphere {
 ...
 public:
  Sphere(const Vec3D &centre = Vec3D(0, 0, 0), double radius = 10.0, const Color &c = Colors::white)
      : centre(centre), radius(radius), color(c) {}
  ...
  double intersects(const Ray &ray) const {
    ...
    if (discriminant < 0) {
      ...
      if (r1 > 0)
        min_t = std::min(min_t, r1);
      if (r2 > 0)
        min_t = std::min(min_t, r2);
      return min_t;
    }
  }
  ...
  static Sphere random() {
    return Sphere(
        Vec3D(mtrand(-200, 200), mtrand(-200, 200), 300 + mtrand(-100, 200)), mtrand(3, 34),
        Colors::random()
    );
  }
};
