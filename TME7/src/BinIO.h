 ...
 * - ptr_to_offset : table de hash associant aux pointeurs de Vertex les offsets dans le fichier où
 * ils ont été sérialisés Retourne l'offset dans le fichier où le noeud a été sérialisé.
 */
off_t serialize(int fd, const Vertex *node,
  ...
  if (ptr_to_offset.contains(node))
  ...
  ptr_to_offset[node] = of;
  // 3. Au bon offset écrire le header (id, child_count)
  write(fd, &node->id, sizeof(node->id));
  size_t child_count = node->children.size();
  write(fd, &child_count, sizeof(child_count));
  // 4. s'assurer de faire grandir le fichier suffisamment pour loger le nouveau noeud mais pas
  // trop. Plusieurs options ici : ftruncate, ou write de zéros (puis seek), ou lseek au delà de la
  // fin et write...
  off_t write_of = lseek(fd, 0, SEEK_CUR);
  ftruncate(fd, write_of + child_count * sizeof(off_t));

  // 5. en boucle sur child_count (lseek/write ou pwrite recommandé),
  // itérer sur les enfants, appeler récursivement serialize pour chaque enfant. (Attention décale
  // le curseur de fd!) Récupérer l'offset retourné, et l'écrire à la bonne position dans le fichier
  // (après le header, dans le tableau d'offsets).
  for (Vertex *c : node->children) {
    off_t c_of = serialize(fd, c, ptr_to_offset);
    lseek(fd, write_of, SEEK_SET);
    write(fd, &c_of, sizeof(c_of));
    write_of += sizeof(off_t);
  }
...
Vertex *deserialize(int fd, off_t offset, std::unordered_map<off_t, Vertex *> &offset_to_vertex,
  ...
  if (offset_to_vertex.contains(offset))
    return offset_to_vertex[offset];
  ...
  size_t child_count;
  read(fd, &child_count, sizeof(child_count));
  ...
  // Pour chaque offset, appeler récursivement deserialize (Attention ça va faire bouger le curseur
  // de fd!) Ajouter les pointeurs vers enfant qui reviennent de la récursion au Vertex en
  // construction.
  off_t child_of = lseek(fd, 0, SEEK_CUR);
  for (size_t i = 0; i < child_count; ++i) {
    off_t of;
    pread(fd, &of, sizeof(of), child_of);
    child_of += sizeof(of);
    v->children.push_back(deserialize(fd, of, offset_to_vertex, graph));
  }
...
Graph parseBin(const std::string &filename) {
  ...
  off_t root_offset = lseek(fd, 0, SEEK_CUR);
