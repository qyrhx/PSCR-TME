...
class TestString {
 public:
  ...
  static const char *getData(const String &s) { return s.data; }
  ...
  static void testMoveAssignment() {
    ...
    test_assert(
        contentsMatch(s7, String("Move Assign Test")), "move assign from return contents match"
    );
    ...
    test_assert(
        contentsMatch(s8, String("Temp Assign")), "move assign from temporary contents match"
    );
  }
